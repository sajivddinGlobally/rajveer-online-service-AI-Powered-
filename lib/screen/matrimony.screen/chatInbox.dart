...
class _ChatinboxState extends ConsumerState<Chatinbox> {
  ...
  void initState() {
    ...
    channel = WebSocketChannel.connect(
      Uri.parse("wss://api.rajveerfacility.in/matrimonies/inbox/ws/$userid"),
    );

    // 📌 Socket se aane wale data ko listen karke print karna
...
class _ChatinboxState extends ConsumerState<Chatinbox> {
  ...
  late WebSocketChannel channel;
  List<dynamic> inboxList = []; // <-- Only Socket Data
  late final userid;
  ...
  void initState() {
    ...
    // Listen socket messages & update UI
    channel.stream.listen((message) {
      log("💬 Socket: $message");
      final jsonData = jsonDecode(message);

      if (jsonData["event"] == "inbox_update") {
        setState(() {
          inboxList = jsonData["inbox"];
        });
      }
    }, onError: (e) {
      log("❌ Socket Error: $e");
  ...
  @override
  Widget build(BuildContext context) {
    final filteredList = inboxList.where((chat) {
      final userName =
      chat["other_user"]["name"].toString().toLowerCase();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          _appBar(),
          if (isShow) _searchBox(),
          SizedBox(height: 25.h),
          _chatList(filteredList),
        ],
      ),
    );
  }
  ...
  Widget _appBar() {
    return Row(
      children: [
        ...
        Text("Chat History",
            style: GoogleFonts.roboto(
                fontSize: 18.w, fontWeight: FontWeight.w600, color: Color(0xff008080))),
        Spacer(),
        InkWell(
  ...
  Widget _chatList(List filteredList) {
    return Expanded(
      child: Container(
        ...
        child: filteredList.isEmpty
          ...
          child: Text("No messages yet",
            ...
            itemBuilder: (context, index) {
              ...
              final user = item["other_user"];

              return ListTile(
                leading: CircleAvatar(
                  backgroundImage: user["profilePick"] != null
